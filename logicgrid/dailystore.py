...
class DuplicateScore(StoreError):
    """This solve (session id) is already on the board."""
...
def insert_score(day: date, name: str, time_ms: int, steps: int | None,
    ...
    """One row per solve. The unique ``sid`` column makes a result token
    single-use — replaying it is a 409, surfaced as DuplicateScore."""
    _request(
        "POST", "daily_scores",
        body={
            "day": day.isoformat(),
